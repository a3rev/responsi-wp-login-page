...
class Responsi_WP_Login_Page {
	...
	public function init() {
		add_action( 'login_enqueue_scripts',array( $this,'global_responsi_options_wp_login_page'), 9 );
		add_action( 'init',array( $this,'global_responsi_options_wp_login_page'), 2 );
	...
	public function responsi_customize_save_options( $settings ) {
		global $responsi_options_wp_login_page, $wp_customize;
		$post_value = array();
		...
		if( is_array( $responsi_options_wp_login_page ) && count( $responsi_options_wp_login_page ) > 0 && is_array( $post_value ) && count( $post_value ) > 0 ){
			...
			$_default_options = responsi_default_options( 'wp_login_page' );
			...
			if( is_array( $_default_options ) && count( $_default_options ) > 0 ){
				...
				if ( is_array($post_value) ) {
		            ...
		            $responsi_options_wp_login_page = array_replace_recursive( $responsi_options_wp_login_page, $post_value );
		        }

				foreach( $responsi_options_wp_login_page as $key => $value ){
				...
				if( $build_sass ) {
					update_option( 'wp_login_page_'.get_stylesheet(), $new_options );
					$this->responsi_dynamic_css();
					do_action( 'wp_login_page_build_dynamic_css_success', $post_value );
				}
			}
		}
	}
	...
	public function global_responsi_options_wp_login_page(){
	    global $responsi_options_wp_login_page, $wp_customize;
	    ...
	    $_wp_login_page_options = $_default_options;

		if ( defined( str_replace("-","_", $_childthemes ) . '_wp_login_page' ) ) {
			if ( function_exists('default_option_child_theme') ){
				$responsi_options_wp_login_page = array_replace_recursive( $_wp_login_page_options, default_option_child_theme() );
			}
		}

		$responsi_mods = get_option( 'wp_login_page_'.$_childthemes, array() );

	    if( is_array( $responsi_mods ) ){
	        $_wp_login_page_options = array_replace_recursive( $_wp_login_page_options, $responsi_mods );
	    }
	    
	    if( 'responsi-blank-child' == $_childthemes ){
	        $responsi_mods = get_option('wp_login_page_responsi', array());
	        $responsi_blank_child =  get_option('wp_login_page_responsi-blank-child', array());
	        if( is_array($responsi_mods) ){
	            $_wp_login_page_options = array_replace_recursive( $_wp_login_page_options, $responsi_mods );
	        }
	        if( is_array($responsi_blank_child) ){
	            $_wp_login_page_options = array_replace_recursive( $_wp_login_page_options, $responsi_blank_child );
	        }
	    }
	    ...
	    if( is_customize_preview() && ( isset( $_REQUEST['changeset_uuid'] ) || isset( $_REQUEST['customize_changeset_uuid'] ) ) ){
	        ...
	        if ( is_array($changeset_data) ) {
	            if( count( $changeset_data ) > 0 ){
	                $_wp_login_page_options_preview = array();
	                foreach ( $changeset_data as $setting_id => $setting_params ){
	                    ...
	                    if ( isset( $setting_params['type'] ) && 'theme_mod' === $setting_params['type'] ) {
							...
							if ( preg_match( $namespace_pattern, $setting_id, $matches ) && get_stylesheet() === $matches['stylesheet'] ) {
								$_wp_login_page_options_preview[ $matches['setting_id'] ] = $setting_params['value'];
							}
						} else {
							$_wp_login_page_options_preview[ $setting_id ] = $setting_params['value'];
						}
	                }
	                $_wp_login_page_options_preview = apply_filters( 'responsi_customized_post_value', $_wp_login_page_options_preview );
	                if ( is_array($_wp_login_page_options) && is_array( $_wp_login_page_options_preview ) && count( $_wp_login_page_options_preview ) > 0 ) {
	                    if ( is_object( $_wp_login_page_options_preview ) ){
	                        $_wp_login_page_options_preview = clone $_wp_login_page_options_preview;
	                    }
	                    $_wp_login_page_options = array_replace_recursive( $_wp_login_page_options, $_wp_login_page_options_preview );
	                }
	            }
	        }
	    }
	    ...
	    if (isset($_POST['customized'])) {
	        ...
	        $post_value = json_decode(wp_unslash($_POST['customized']), true);
	        $post_value = apply_filters('responsi_customized_post_value', $post_value);
	        if ( is_array($_wp_login_page_options) && is_array($post_value) ) {
	            if ( is_object( $post_value ) ){
	                $post_value = clone $post_value;
	            }
	            $_wp_login_page_options = array_replace_recursive($_wp_login_page_options, $post_value);
	        }

	    }

	    $responsi_options_wp_login_page = $_wp_login_page_options;

	    return $responsi_options_wp_login_page;
	}
	...
	public function responsi_build_dynamic_css( $preview = false ) {
	    ...
	    if ( !$preview ) {
	        $responsi_options_wp_login_page = $this->global_responsi_options_wp_login_page();
	    } else {
	    ...
	    $responsi_login_page_container_border_radius        = $responsi_options_wp_login_page['responsi_login_page_container_border_radius'];
	    ...
	    $container_css .= responsi_generate_border($responsi_login_page_container_border, 'border');
	    $container_css .= responsi_generate_border_radius($responsi_login_page_container_border_radius);
