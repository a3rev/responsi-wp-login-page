...
define( 'RESPONSI_WPLOGIN_VERSION', '1.3.9' );

function responsi_wp_login_page_activate_validate() {
    if ( 'responsi' !== get_template() ) {
        echo sprintf( __( 'This is a plugin for Responsi Framework, you need to install <a href="%s" target="_blank" rel="noopener">Responsi Framework</a> theme from WordPress first before can activate this.', 'responsi-wp-login-page' ), 'https://wordpress.org/themes/responsi-framework/' );
        die();
    }
...
function responsi_addon_wp_login_page_upgrade_version() {
	...
	if ( version_compare(get_option('a3rev_responsi_wp_login_page_version'), '1.3.9') === -1 ) {
